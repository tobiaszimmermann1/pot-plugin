  ...
  return (
    <>
      <div className="fc_filters_breadcrumb">
        <div>
          <span className="fc_filters_link">
            <Link to={`/`}>{__("Kategorien", "fcplugin")}</Link>
          </span>
          <span style={{ margin: "0 10px" }}>&#8594;</span>{" "}
          <span>
            <Link to={`/category/${product.category_name.replace(/\//g, "-")}`}>{product.category_name.replace(/\//g, "-")}</Link>
          </span>
          <span style={{ margin: "0 10px" }}>&#8594;</span> <span className="fc_filters_link">{product.name}</span>
        </div>
      </div>
      ...
      <div className="fc_product_details_wrapper">
        ...
        <div className="fc_product_details_content">
          ...
          <p>
            {product._einheit} | <span dangerouslySetInnerHTML={{ __html: currency }} /> {parseFloat(product.price).toFixed(2)}
