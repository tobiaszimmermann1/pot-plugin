...
function SelfCheckoutProducts({ setModalClose, prods }) {
  ...
  useEffect(() => {
    if (prods) {
      axios
        .get(`${appLocalizer.apiUrl}/foodcoop/v1/getOption?option=fc_self_checkout_products`, {
          headers: {
  ...
  const handleSubmit = () => {
    ...
    axios
      .post(
        `${appLocalizer.apiUrl}/foodcoop/v1/postSaveProductsSelfCheckout`,
        {
      ...
      <Dialog fullScreen open={true} maxWidth="lg" scroll="paper" aria-labelledby="scroll-dialog-title" aria-describedby="scroll-dialog-description">
        <AppBar sx={{ position: "relative", paddingTop: "32px" }}>
          <Toolbar sx={{ justifyContent: "space-between" }}>
            <DialogTitle textAlign="left">{__("Produkte für Self Checkout wählen", "fcplugin")}</DialogTitle>
            <DialogActions>
        ...
        >
          <Alert sx={{ marginBottom: 1 }} severity="info">
            {__("Produkte auswählen, die im Self Checkout verfügbar sind, dann speichern.", "fcplugin")}
          </Alert>
          <MaterialReactTable
            muiTablePaperProps={{
              elevation: 0,
...
export default SelfCheckoutProducts
