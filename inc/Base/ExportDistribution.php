...
class ExportDistribution extends BaseController
{
    public function register()
    {
        add_action( 'wp_ajax_nopriv_fc_export_distribution_lists_function', array($this,'fc_export_distribution_lists_function' ) );
        add_action( 'wp_ajax_fc_export_distribution_lists_function', array($this,'fc_export_distribution_lists_function' ) );
    }


    function fc_export_distribution_lists_function() {
        ...
        foreach( $lieferanten as $lieferant ){
            ...
            <div style="margin-top:2cm;font-size:18pt;width: 100%;">
                <table style="width: 100%;font-size:8pt;border:1px solid #8e8e8e;" cellspacing="0">
                <tr>
                    <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-weight:bold;">Bestellrunde</td>
                    <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-weight:bold;">'. esc_attr($bestellrunde_name).'</td>
                </tr>
                <tr>
                    <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-weight:bold;">Lieferant</td>
                    <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-weight:bold;">'.esc_attr($lieferant).'</td>
                </tr> 
                <tr>
                    <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-size:8pt;">Bearbeitet von (Visum)</td>
                    <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-size:8pt;"></td>
                </tr>
                </table>
            </div>
            ...
            <div style="margin-top:1cm;font-size:18pt;width: 100%;">
                <table style="width: 100%;border:1px solid #8e8e8e;" cellspacing="0">
                <tr>
                    <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-size:8pt;">Produkt</td>
                    <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-size:8pt;">Einheit</td>
            ';
            ...
            foreach ($lieferant_users as $user => $user_id ) {
                $body .= '
                <td style="padding:5px 5px 5px 5px;border:1px solid #8e8e8e;font-weight:bold;font-size:8pt;text-rotate:90;width:0.5cm;text-align:left;">'.$user.'</td>
                ';
            }

            $body .= '
            <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-size:8pt;width:0.5cm;">AK</td>
            <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-size:8pt;width:5cm;">Notiz</td>
            </tr>
            ';
            ...
            foreach( $produkte as $produkt_id => $produkt ){
                ...
                if ($product_lieferant == $lieferant) {
                    ...
                    $body .= '
                    <tr>
                    <td style="padding:5px 20px 5px 10px;border:1px solid #8e8e8e;font-size:6pt;">'.$product_name.'</td>
                    <td style="padding:5px 20px 5px 10px;border:1px solid #8e8e8e;font-size:6pt;">'.$product_einheit.'</td>
                    ';

                    $i = 0;
                    while ($i < $num_users) {

                        $user_id = array_values($lieferant_users)[$i];
        
                        $body .= '
                            <td style="padding:5px 5px 5px 5px;border:1px solid #8e8e8e;font-weight:bold;font-size:6pt;text-align:center;width:0.5cm;">
                        ';
                    ...
                    $body .= '
                        <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-size:6pt;"></td>
                        <td style="padding:10px 20px 10px 10px;border:1px solid #8e8e8e;font-size:6pt;"></td>
                    </tr>
                    ';
            ...
            $mpdf->Output($path.$export_name.'/'.esc_attr($bestellrunde_name).'-'.esc_attr($lieferant).'.pdf', \Mpdf\Output\Destination::FILE);
        ...
        // Zip the PDF's
        $zip = new ZipArchive();
        $zip->open($path.$export_name.'/'.$bestellrunde_name.'-verteil-listen.zip', ZipArchive::CREATE | ZipArchive::OVERWRITE);
        ...
        echo json_encode($this->upload_dir['url'].'/'.$export_name.'/'.$bestellrunde_name.'-verteil-listen.zip');
        die();
