...
foreach ($import_products as $import_product) {
  if ($i > 1) {
    ...
    /*
    // if product id was not found, but the product has sku specified, check if it exists in db
    if (!$product_exists && !empty($import_product[11])) {
      $sku = sanitize_text_field($import_product[11]);
      if (array_key_exists($sku, $all_products_by_sku)) {
        $product_exists = true;
        $product_id = $all_products_by_sku[$sku]->get_id();
      }
    }
    */
    ...
    elseif ($import_product[16] === "-1.000" || $import_product[16] === "-1.00" || $import_product[16] === "-1.0" || $import_product[16] === "-1" || $import_product[16] === -1) {
      ...
      // if stock import not selected, disable stock management for the product
      update_post_meta( $product_id, "_stock_status", "instock" );
      update_post_meta( $product_id, "_manage_stock", "no" );
    }
