  ...
  function postImportProducts($data) {
    ...
    foreach($products as $product) {
      ...
      if ($check_if_product_exists) {
        ...
        if ($product[9] != "") {
          // check if current product has an attachment already
          ...
          if ($current_attachment) {
            ...
            // check if imported image is already in media gallery
            global $wpdb;
            $filename = basename($product[9]);
            $existing_image_id = intval( $wpdb->get_var( "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_value LIKE '%/$filename'" ) );
