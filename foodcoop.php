...
function fc_admin_load_scripts() {
  ...
  wp_enqueue_script( 'fc-script', plugin_dir_url( __FILE__ ) . 'build/backend.js?version=1.7.8', array( 'wp-element', 'wp-i18n' ), '1.0', false );
...
function fc_wp_load_scripts() {
  ...
  if (get_option('fc_enable_rounds_storewide')) {
    wp_enqueue_script( 'fc-script-sitewide-bestellrunden', plugin_dir_url( __FILE__ ) . 'scripts/sitewide-bestellrunden.js?version=1.7.8', array( 'jquery' ), '1.0', false );
  }
  wp_enqueue_script( 'fc-script-frontend', plugin_dir_url( __FILE__ ) . 'build/frontend.js?version=1.7.8', array( 'wp-element', 'wp-i18n' ), '1.0', false );
      ...
      ?>
        <div class="admin-alert">
          <a href="<?php echo get_site_url(); ?>/wp-admin/admin.php?page=foodcoop-plugin">
            <?php echo __("Hallo Admin! Zur POT Plugin Administration", "fcplugin"); ?> >>
          </a>
        </div>
      <?php
    }
